...
- Contact rewards now use Isaac Lab contact time instead of height thresholds; foot slip is actual-contact times planar foot velocity, and the torque curriculum starts at `3.0` while still applying both actuator-side and PhysX-side effort limits.
- Teacher and student output-head initialization scales are configurable and default to `0.1` instead of the previous near-zero `0.01` hardcode.
