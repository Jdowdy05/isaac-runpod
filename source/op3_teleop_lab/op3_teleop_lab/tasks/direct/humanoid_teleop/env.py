...
from .env_cfg import compute_actor_frame_dim
...
class HumanoidTeleopEnv(DirectRLEnv):
    def __init__(self, cfg, render_mode: str | None = None, **kwargs):
        ...
        if missing_joint_names:
            raise ValueError(
                "Could not resolve all humanoid teleop joints from the final asset. "
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        fallen = self._compute_fallen_mask(projected_gravity)
        time_out = self._compute_time_out_mask()
        termination_penalty_mask = (fallen & ~time_out).float()
    ...
    def _compute_fallen_mask(self, projected_gravity: torch.Tensor | None = None) -> torch.Tensor:
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None) -> None:
        ...
        yaw_quat = quat_from_euler_xyz(torch.zeros_like(yaw_noise), torch.zeros_like(yaw_noise), yaw_noise)
        ...
        if self._fixed_joint_ids:
            self.robot.write_joint_state_to_sim(fixed_joint_pos, fixed_joint_vel, joint_ids=self._fixed_joint_ids, env_ids=env_ids)
    ...
    def _compute_add_differential(self) -> torch.Tensor:
        ...
        return torch.cat(pos_diffs + rot_diffs, dim=-1)
